...
import (
	...
	"github.com/sanrentai/gkvdb/gkvdb/gfilespace"
)
...
func (db *DB) newTable(name string) (*Table, error) {
	...
	if gfile.Size(ixpath) == 0 {
		gfile.PutBytes(ixpath, make([]byte, gINDEX_BUCKET_SIZE*gDEFAULT_PART_SIZE))
	}
	// 初始化相关服务
	table.initFileSpace()
...
func (table *Table) get(key []byte) []byte {
	...
	if v, _ := table.cache.Get(ckey); v != nil {
		return v.([]byte)
	}
	table.mu.RLock()
...
func (table *Table) items(max int, m map[string][]byte) map[string][]byte {
	...
	ixbuffer := gfile.GetBytes(table.getIndexFilePath())
	for i := 0; i < len(ixbuffer); i += gINDEX_BUCKET_SIZE {
		...
		if mtbuffer := gfile.GetBytesByTwoOffsets(mtpf, int64(mtindex), int64(mtindex+mtsize)); len(mtbuffer) > 0 {
			for i := 0; i < len(mtbuffer); i += gMETA_ITEM_SIZE {
				...
				if klen > 0 && vlen > 0 {
					...
					data := gfile.GetBytesByTwoOffsets(dbpf, dbstart, dbend)
					keyb := data[1 : 1+klen]
					key := string(keyb)
					m[key] = data[1+klen:]
					if len(m) == max {
...
func (table *Table) getIndexInfoByRecord(record *_Record) error {
	...
	for {
		if buffer := gfile.GetBytesByTwoOffsets(pf, record.index.start, record.index.end); buffer != nil {
			bits := gbinary.DecodeBytesToBits(buffer)
...
func (table *Table) getDataInfoByRecord(record *_Record) error {
	...
	if record.meta.buffer = gfile.GetBytesByTwoOffsets(pf, record.meta.start, record.meta.end); record.meta.buffer != nil {
		// 二分查找
		min := 0
...
func (table *Table) getDataByOffset(start, end int64) []byte {
	if end > 0 {
		...
		defer pf.Close()
		buffer := gfile.GetBytesByTwoOffsets(pf, start, end)
		if buffer != nil {
			return buffer
		}
	}
