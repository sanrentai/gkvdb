...
func (table *Table) autoCompactingData() error {
	...
	if dbstart == dbsize {
		...
		if dbpf, retmsg := table.getDataFilePointer(); retmsg == nil {
			...
			if buffer := gfile.GetBytesByTwoOffsets(dbpf, dbstart, dbstart+1); buffer != nil {
				klen := gbinary.DecodeToUint8(buffer)
				key := gfile.GetBytesByTwoOffsets(dbpf, dbstart+1, dbstart+1+int64(klen))
				record := &_Record{
				...
				if retmsg = table.getIndexInfoByRecord(record); retmsg == nil {
					if record.meta.end > 0 {
						if retmsg = table.getDataInfoByRecord(record); retmsg == nil {
							if dbbuffer := gfile.GetBytesByTwoOffsets(dbpf, record.data.start, record.data.end); dbbuffer != nil {
	...
	if mtstart == mtsize {
		if err := os.Truncate(table.getMetaFilePath(), int64(index)); err == nil {
			if index == 0 {
				...
				gfile.PutBytes(table.getIndexFilePath(), make([]byte, gINDEX_BUCKET_SIZE*gDEFAULT_PART_SIZE))
			}
			return nil
		} else {
			return err
		...
		if mtpf, retmsg := table.getMetaFilePointer(); retmsg == nil {
			...
			if buffer := gfile.GetBytesByTwoOffsets(mtpf, mtstart, mtstart+gMETA_ITEM_SIZE); buffer != nil {
				...
				// 查找对应的索引信息，并执行更新
				if retmsg = table.getIndexInfoByRecord(record); retmsg == nil {
					if mtbuffer := gfile.GetBytesByTwoOffsets(mtpf, record.meta.start, record.meta.end); mtbuffer != nil {
